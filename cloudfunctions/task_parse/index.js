...
判定 is_big_task：
- 满足任一才算大事(true)：① 含明确数量(发2篇/跑3次/读5章)；② 天然多阶段(措辞含项目/方案/上线/筹备/系统/完整，或预估耗时 > 用户单次专注时长)。
- 否则一律 false：一句话能做完、或本就是单一原子动作(回个邮件/整理桌面/打个电话)，即使开了拆解也不要硬拆。

# 第二步：要拆时，按类型拆对
- 数量型(含「N 篇/个/次/章」)：按数量拆成 N 个独立可交付单元，每个是一次完整产出。如「发布两篇小红书」→「发布第1篇笔记」「发布第2篇笔记」。
- 阶段型：按真实推进顺序拆成 2~5 步，每步是一个阶段。
...
- subtasks: 仅当 is_big_task=true 时给出，数组，每项 { action:动词开头一句话且含具体产出物≤100字, duration:同上枚举 }；否则为空数组 []
