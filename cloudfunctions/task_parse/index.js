...
# 第一步：判断要不要拆
拆解的价值是「替用户消除决策」，不是「把步骤切小」。宁可不拆，也不要凑无意义的步骤。
判定 is_big_task：
- 满足任一才算大事(true)：① 含明确数量(发2篇/跑3次/读5章)或模糊复数(做几个视频/写一些文案/改好几处)；② 天然多阶段(措辞含项目/方案/上线/筹备/系统/完整，或预估耗时 > 用户单次专注时长)。
- 否则一律 false：一句话能做完、或本就是单一原子动作(回个邮件/整理桌面/打个电话)，即使开了拆解也不要硬拆。

# 第二步：要拆时，按类型拆对，步数随任务规模走（不要凑数，也不要硬压）
- 数量型：含明确数字(N篇/个/次/章)按数量拆成 N 个独立可交付单元；含模糊复数(几个/一些/多个/好几)先按 3 个估，每个是一次完整产出。如「做几个剪辑视频」→「剪好第1个视频」「剪好第2个视频」「剪好第3个视频」。
...
- subtasks: 仅当 is_big_task=true 时给出，数组，步数由任务复杂度决定(数量型=数量，阶段型简单2~3步/复杂6~8步)，每项 { action:动词开头一句话且含具体产出物≤100字, duration:同上枚举 }；否则为空数组 []
...
exports.main = async (event) => {
  ...
  // 子任务清洗：限粒度、归一化耗时、最多 8 步（按规模不锁死，仅防 AI 失控）。开启拆分或强制规划时生效
  let subtasks = [];
